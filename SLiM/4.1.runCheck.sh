...
CHECK=4.runCheck.txt
echo -e "CHR\tSIM\tRUN\tCYCLE\tSAMPLE\tBED\tVCF\tHET" > "${CHECK}"
            ...
            for CYCLE in $(seq 5 5 20); do
            	...
            	if [[ -f "${VCF}" ]]; then
					...
					for SAMPLE in "${samples[@]}"; do
					    ...
					    HET="HET/noROH_indiv_${BFILE}.het"

					    [[ -f "${BED}" ]] && BED_CHECK="Yes" || BED_CHECK="No"
					    [[ -f "${ROH_VCF}" ]] && VCF_CHECK="Yes" || VCF_CHECK="No"
					    [[ -f "${HET}" ]] && HET_CHECK="Yes" || HET_CHECK="No"

					    echo -e "A1\t${SIM_NAME}\t${RUN_NAME}\t${CYCLE}\t${SAMPLE}\t${BED_CHECK}\t${VCF_CHECK}\t${HET_CHECK}" >> "${CHECK}"
					done
...
echo "COMPLETE $(date): ${CHR}"
echo "-----"
echo

echo "ALL DONE. Proceed to SLiM_nonROH-het-per-kb.R">> ${LOG}
